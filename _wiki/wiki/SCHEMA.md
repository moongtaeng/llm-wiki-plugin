...
- `plugin-design` — decisions and rationale about this repo's own architecture (skill prose, scripts, commands).
- `retrieval` — anything about the search/embedding/BM25/RRF pipeline.
- `external-ref` — notes distilled from outside sources (gists, blog posts, comparable projects) rather than this repo's own code.
